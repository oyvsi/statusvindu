...
function check_sensor($query, $type) {
	if($dummydata != TRUE) {
...
if(isset($_GET['type'])) {
	...
	if($_GET['type'] == 'Temperature') {
			...
			echo json_encode(check_sensor($query, 'Temperature'), JSON_NUMERIC_CHECK);
	} 
	elseif ($_GET['type'] == 'Humidity') {
		...
		echo json_encode(check_sensor($query, 'Humidity'), JSON_NUMERIC_CHECK);
	}
}
